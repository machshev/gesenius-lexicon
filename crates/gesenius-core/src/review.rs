...
history.replaceState(null,'',destination('/pages',{edition:page.edition,source_page:page.source_page}));
...
async function loadEntry(id){current=await (await fetch('/api/entries/'+encodeURIComponent(id)+'?edition='+encodeURIComponent($('#edition').value))).json();history.replaceState(null,'',destination('/entries',{edition:current.edition,entry:current.id}));await render();}
