...
if (typeof module !== 'undefined') module.exports = { edit, backspace, fromCodePoint, layouts, ethiopicRows, syriacKeyForm };
...
function renderKeys() {
    ...
    const makeKey = ([text, name]) => {
        ...
        button.textContent = (isMark(text) ? '◌' : '') + (layout[0] === 'syriac' ? syriacKeyForm(text, el('syriacForm').value) : text);
...
function changeLanguage() {
    ...
    el('syriacFormControl').hidden = layout[0] !== 'syriac';
...
el('syriacForm').onchange = renderKeys;
el('syriacFont').onchange = () => {
    window.SyriacStyle.set(el('syriacFont').value);
};
el('syriacFont').value = window.SyriacStyle.get();
